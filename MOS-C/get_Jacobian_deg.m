...
V_arr=([V_L,V]+[V,V_R])/2;
arr1=(EF_deg-Ec+V_arr*VN)/kT;
Dn_by_mun_array=V_thermal*fermi(0.5,arr1)./fermi(-0.5,arr1);
V_thermal_eff=Dn_by_mun_array;

%%%%%
% V_all=[V_L,V,V_R]*VN;
% V_diff=diff(V_all);
% arr1=(EF_deg-Ec+V_all)/kT;
% arr2=fermi(.5,arr1);
% 
% zz=V_diff.*(log(arr2(2:end)./arr2(1:end-1))).^-1;
% zz(isnan(zz))=V_thermal; zz(isinf(zz))=V_thermal;
%V_thermal_eff=zz;
%%%%%
...
mu_n = fun_get_mobility(mu0_n, 2, v_sat, E);   Dn=mu_n.*V_thermal_eff; 
mu_p = fun_get_mobility(mu0_p, 1, v_sat, E);  Dp=mu_p.*V_thermal_eff; 

mu_n_dash = fun_get_mobility_der(mu0_n, 2, v_sat, E);
mu_p_dash = fun_get_mobility_der(mu0_p, 1, v_sat, E);

U_diff=diff([V_L,V,V_R]).*(VN./V_thermal_eff);
B_U_diff=B(U_diff);
...
% hn=Dn.*([n_L,n].*Bdash_minusU_diff + [n,n_R].*Bdash_U_diff); hn=hn/D0_n; hn=hn.*VN./V_thermal_eff;
% hn=hn-0*(mu_n_dash.*Jn./mu_n)*VN/(N1*q*D0_n);
% hn=hn*tn;

hn=-tn*Dn.*B_U_diff.*[n,n_R]./V_thermal_eff*(VN/D0_n); %equilibrium case
...
%hp=Dp.*([p_L,p].*Bdash_U_diff + [p,p_R].*Bdash_minusU_diff); hp=hp/D0_p; hp=hp.*VN./V_thermal_eff;
%hp=hp-0*(mu_n_dash.*Jn./mu_n)*V_thermal/(N*q);
%hp=-tp*hp;

hp=tp*Dp.*B_minusU_diff.*[p,p_R]./V_thermal_eff*(VN/D0_p); %equilibrium case
